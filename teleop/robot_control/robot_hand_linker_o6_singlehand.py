...
class O6_Controller:
    ...
    def _subscribe_hand_state(self):
        ...
        while True:
            # Left Hand
            left_state_msg = self.left_hand_api.get_state() if self.left_hand_api is not None else None
            ...
            # Right Hand
            right_state_msg = self.right_hand_api.get_state() if self.right_hand_api is not None else None
    ...
    def _send_hand_command(left_hand_api, right_hand_api, left_angle_cmd_scaled, right_angle_cmd_scaled):
        ...
        if left_hand_api is not None:
            left_hand_api.finger_move(pose=left_angle_cmd_scaled)
        if right_hand_api is not None:
    ...
    def control_process(self, left_hand_array, right_hand_array, left_hand_state_array, right_hand_state_array,
        ...
        left_hand_api, right_hand_api =self.init_hand_api(self.left_can_port, self.right_can_port)
        ...
        while self.running:
            ...
            left_hand_state = left_hand_state_array[:] if left_hand_api is not None else np.zeros(O6_Num_Motors)
            right_hand_state = right_hand_state_array[:] if right_hand_api is not None else np.zeros(O6_Num_Motors)

            state_data = np.concatenate((np.array(left_hand_state), np.array(right_hand_state)))
            ...
            if not np.all(right_hand_data == 0.0) or not np.all(left_hand_data == 0.0): # if hand data has been initialized.
                ...
                if left_hand_api is None:
                    left_q_target = np.zeros(O6_Num_Motors)
                if right_hand_api is None:
                    right_q_target = np.zeros(O6_Num_Motors)
